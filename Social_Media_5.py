...
def login():
    ...
    login_window.configure(bg="light pink")  # Changed background color

    frame = tk.Frame(login_window, bg="light pink")  # Changed background color
    frame.pack(expand=True)

    tk.Label(frame, text="Email", font="Bold", bg="light pink").pack()  # Changed background color
    email_entry = tk.Entry(frame)
    email_entry.pack(padx=10, pady=7)

    tk.Label(frame, text="Password", font="Bold", bg="light pink").pack()  # Changed background color
    password_entry = tk.Entry(frame, show="*")
    password_entry.pack(padx=10, pady=7)

    error_label = tk.Label(frame, text="", fg="red", bg="light pink")  # Changed background color
    error_label.pack()
    ...
    tk.Button(frame, text="Login", font="Bold", fg="black", bg="light gray", command=validate_login).pack(pady=5)
    tk.Button(frame, text="Register", font="Bold", fg="black", bg="light gray",command=lambda: [login_window.destroy(), register()]).pack(pady=5)
...
def register():
    ...
    register_window.configure(bg="light pink")  # Changed background color

    frame = tk.Frame(register_window, bg="light pink")  # Changed background color
    frame.pack(expand=True)

    tk.Label(frame, text="Name", font="Bold", bg="light pink").pack()  # Changed background color
    ...
    tk.Button(frame, text="Register", fg="black", width=10, font="Bold", bg="light gray", command=save_registration).pack(pady=5)  # Changed button color
...
def home_page(main_window, s_manager, user_name):
    ...
    home_frame = tk.Frame(main_window, bg="lavender")
    home_frame.pack(pady=20)

    button_style = {"font": ("Times New Roman", 13, "bold"), "fg": "white", "bg": "#9B59B6"}
    # Buttons to navigate to different windows
    add_post_button = tk.Button(home_frame, text="Add Post", command=lambda: add_post_form(main_window, s_manager, user_name), **button_style)
    add_post_button.grid(row=0, column=0, padx=5, pady=10)

    search_button = tk.Button(home_frame, text="Search", command=lambda: open_search(main_window, s_manager, user_name), **button_style)
    search_button.grid(row=0, column=1, padx=5, pady=10)

    profile_button = tk.Button(home_frame, text="Profile", command=lambda: user_profile_page(main_window, s_manager, user_name), **button_style)
    profile_button.grid(row=0, column=2, padx=5, pady=10)

    messages_button = tk.Button(home_frame, text="Messages", command=lambda: open_messages(main_window, s_manager, user_name), **button_style)
    messages_button.grid(row=0, column=3, padx=5, pady=10)

    requests_button = tk.Button(home_frame, text="Friend Requests", command=lambda: open_friend_requests(main_window, s_manager, user_name), **button_style)
    requests_button.grid(row=0, column=4, padx=5, pady=10)


    # Add the Notifications button (as per your request)
    notifications_button = tk.Button(home_frame, text="Notifications", command=lambda: open_notifications(main_window, s_manager, user_name), **button_style)
    notifications_button.grid(row=1, column=1, padx=5, pady=10)
...
def add_post_form(main_window,s_manager,user_name):
    ...
    back_button = ttk.Button(button_frame, text="Back",command=lambda: home_page(main_window, s_manager, user_name), style="Custom.TButton")
...
def user_profile_page(main_window, s_manager, user_name):
    ...
    back_button = tk.Button(main_window, text="Back", command=lambda: home_page(main_window, s_manager, user_name),**button_style)
...
def open_profile_view(profile_window, s_manager, username, posts):
    ...
    # back_button = tk.Button(profile_window, text="Back",command=lambda: s_manager.previous_view(profile_window, s_manager, username), **button_style)
    # back_button.place(relx=0.0, rely=0.0, anchor="nw")

    tk.Label(profile_window, text=f"{username}'s Profile", font=("Times New Roman", 16, "bold", "italic"),fg="#301934",bg="lavender").pack(pady=20)
...
def open_search(new_window, s_manager, user_name):
    ...
    back_button = tk.Button(new_window, text="Back", command=lambda: home_page(new_window, s_manager, user_name), **button_style)
    back_button.place(relx=0.0, rely=0.0, anchor="nw")
...
def open_friend_requests(new_window, s_manager, user_name):
    ...
    # Function to refresh the friend request list
...
def open_friend_chat(chat_window ,s_manager, user_name, friend):
    ...
    button_style = {"font": ("Times New Roman", 13, "italic"), "fg": "white", "bg": "#9B59B6"}
    back_button = tk.Button(chat_window, text="Back",command=lambda:open_messages(chat_window,s_manager, user_name),**button_style)
    back_button.place(relx=0.0, rely=0.0, anchor="nw")
...
def open_comments(s_manager, post, user_name):
    ...
    s_manager.navigation.add_page(new_window)
...
def quick_sort_notifications(notifications):
    ...
    pivot_time = datetime.strptime(pivot['time'], "%Y-%m-%dT%H:%M:%S")
    left = [x for x in notifications if datetime.strptime(x['time'], "%Y-%m-%dT%H:%M:%S") > pivot_time]
    middle = [x for x in notifications if datetime.strptime(x['time'], "%Y-%m-%dT%H:%M:%S") == pivot_time]
    right = [x for x in notifications if datetime.strptime(x['time'], "%Y-%m-%dT%H:%M:%S") < pivot_time]
    return quick_sort_notifications(left) + middle + quick_sort_notifications(right)

def open_notifications(main_window, s_manager, user_name):
    new_window = tk.Toplevel()
    ...
    # Collect all notifications (likes and comments) with their timestamps
    notifications = []
    ...
    # Sort notifications by time using quick sort
    sorted_notifications = quick_sort_notifications(notifications)

    # Frame to hold notifications
    ...
    for notif in sorted_notifications:
        # Create a purple frame for each notification
        post_notif_frame = tk.Frame(notif_frame, bg="white", pady=5)  # Set background to purple
        post_notif_frame.pack(fill=tk.X, pady=5)

        notification_text = f"{notif['user']} {notif['action']} at {notif['time']}"
        tk.Label(post_notif_frame, text=notification_text, font=("Arial", 12), fg="purple", bg="white").pack(side=tk.LEFT)  # Change label fg to white

        # Add a button to view the post related to this notification
    ...
    # Back button
    tk.Button(new_window, text="Back", **button_style, command=lambda: home_page(main_window, s_manager, user_name)).pack(pady=10)

def open_post_view(parent_window, post, s_manager):
    # Close the current window
    parent_window.withdraw()

    # Create a new window for the post
    post_window = tk.Toplevel()
    SMM.navigation.add_page(post_window)

    post_window.title(f"Post by {post['author']}")
    post_window.geometry("500x500")
    ...
    # Display post content based on the type
    if post['type'] == "text":
    ...
    tk.Button(post_window, text="Back", font=("Arial", 12), fg="white", bg="purple",
              command=lambda: go_back_to_notifications(post_window, parent_window)).pack(pady=10)

def go_back_to_notifications(post_window, parent_window):
    post_window.destroy()

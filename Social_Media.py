...
def home_page(main_window, s_manager, user_name):
    ...
    # Create a frame for the buttons and align them horizontally
    button_frame = tk.Frame(main_window, bg="lavender")
    button_frame.pack(pady=10, fill="x")

    button_style = {"font": ("Times New Roman", 12, "bold"), "fg": "white", "bg": "#9B59B6"}

    # Align the buttons in one row using pack() with side="left"
    ...
    # Create a frame for the posts and center them
...
def quick_sort_notifications(notifications):
    ...
    pivot_time = datetime.strptime(pivot['time'], "%Y-%m-%d %H:%M:%S")  # Updated format
...
def open_notifications(new_window, s_manager, user_name):
    ...
    for notif in sorted_notifications:
        # Create a white frame for each notification
...
def open_post_view(post_window, post, s_manager):
    ...
    button_style = {"font": ("Times New Roman", 13, "italic"), "fg": "white", "bg": "#9B59B6"}
    back_button = tk.Button(post_window, text="Back",command=lambda:open_notifications(post_window,s_manager,post['author']),**button_style)
    back_button.place(relx=0.0, rely=0.0, anchor="nw")
    # Post content
...
def open_saved_posts(new_window, s_manager, user_name):
    ...
    # Function to refresh the saved posts page
    ...
    # Initial call to populate the saved posts page (auto-refresh like home page)
    refresh_saved_posts()
